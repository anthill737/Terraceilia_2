...
# Preload scripts
const MarketShocksScript = preload("res://scripts/market_shocks.gd")
const ProsperityMeterScript = preload("res://scripts/prosperity_meter.gd")

# Household spawning configuration
const HOUSEHOLD_SPAWN_RADIUS_MIN: float = 100.0  # Min distance from market
const HOUSEHOLD_SPAWN_RADIUS_MAX: float = 200.0  # Max distance from market
const MIN_DISTANCE_FROM_HOUSEHOLDS: float = 50.0  # Avoid overlap
...
var shocks = null  # Market shock system (MarketShocks instance)
var prosperity: ProsperityMeter = null  # Prosperity-driven growth system
...
func _ready() -> void:
	...
	shocks = MarketShocksScript.new()
	shocks.name = "MarketShocks"
	add_child(shocks)
	shocks.set_event_bus(bus)
	
	prosperity = ProsperityMeterScript.new()
	prosperity.name = "ProsperityMeter"
	add_child(prosperity)
	...
	# Wire calendar day_changed to shocks for shock updates
	calendar.day_changed.connect(shocks.set_day)
	# Wire calendar day_changed to prosperity-driven population growth
	calendar.day_changed.connect(_on_day_changed_prosperity_growth)
	
	# Initialize household tracking with baseline household
	households.append(household_agent)
	
	# Wire prosperity meter to systems
	prosperity.bind_references(bus, market, households)
	...
	# Wire market shocks to market and fields
	market.market_shocks = shocks
	field1_plot.bind_shocks(shocks)
	field2_plot.bind_shocks(shocks)
	
	# Wire market shocks to food stockpiles
	farmer.food_stockpile.bind_shocks(shocks)
	baker.food_stockpile.bind_shocks(shocks)
	household_agent.food_stockpile.bind_shocks(shocks)
...
func _on_tick(tick: int) -> void:
	...
	# Update tick for all households
	for household in households:
		if household != null:
			household.set_tick(tick)
...
func get_ui_labels() -> void:
	...
	# Bind prosperity metrics
	if has_node("UI/HUDRoot/Layout/Sidebar/SidebarVBox/ProsperityCard"):
		var prosperity_card = get_node("UI/HUDRoot/Layout/Sidebar/SidebarVBox/ProsperityCard/ProsperityVBox")
		if prosperity_card:
...
func update_ui() -> void:
	...
	# Update prosperity metrics
	if prosperity and prosperity_score_label:
		var score = prosperity.prosperity_score
		var threshold = prosperity.PROSPERITY_THRESHOLD_TO_GROW
		var status = "✓" if score >= threshold else "✗"
		prosperity_score_label.text = "Overall: %.2f / %.2f %s" % [score, threshold, status]
	
	if prosperity and wealth_score_label:
		var wealth = prosperity._compute_wealth_health()
		wealth_score_label.text = "  💰 Wealth: %.2f (%.0f/capita)" % [wealth, prosperity.WEALTH_TARGET]
	
	if prosperity and food_score_label:
		var food = prosperity._compute_food_security()
		food_score_label.text = "  🍞 Food: %.2f" % food
	
	if prosperity and starvation_score_label:
		var starv = prosperity._compute_starvation_pressure()
		starvation_score_label.text = "  ⚠️ Starvation: %.2f (lower=better)" % starv
	
	if prosperity and trade_score_label:
		var trade = prosperity._compute_trade_activity()
		trade_score_label.text = "  📈 Trade: %.2f ($%.0f target)" % [trade, prosperity.TRADE_VALUE_TARGET]
	
	if population_info_label:
		var pop_count = households.size()
		var plural = "household" if pop_count == 1 else "households"
		population_info_label.text = "Population: %d %s" % [pop_count, plural]


func _on_day_changed_prosperity_growth(day: int) -> void:
	"""Prosperity-driven population growth handler."""
	if prosperity == null:
		return
	
	# Update prosperity meter with current economic conditions
	prosperity.update_prosperity(day)
	
	# Check if prosperity allows household spawning
	if prosperity.should_spawn_household(day):
		_spawn_new_household(day)
		prosperity.record_spawn(day)


func _spawn_new_household(day: int) -> void:
	"""Spawn a new household identical to the baseline household using PackedScene."""
	# Get the baseline household as template
	var baseline = household_agent
	if baseline == null:
		if bus:
			bus.log("ERROR: Cannot spawn household - baseline household not found")
		return
	
	# Get market node for positioning
	var market_node = get_node("MarketNode")
	if market_node == null:
		if bus:
			bus.log("ERROR: Cannot spawn household - MarketNode not found")
		return
	
	# Find valid spawn position (avoid overlaps)
	var spawn_pos: Vector2 = _find_valid_spawn_position(market_node.position)
	
	# Create home location node for new household
	var new_home = Node2D.new()
	new_home.name = "HouseholdHome_%d" % (households.size() + 1)
	new_home.position = spawn_pos
	add_child(new_home)
	
	# CRITICAL: Use PackedScene instead of duplicate() for reliable cloning
	var packed = PackedScene.new()
	var pack_result = packed.pack(baseline)
	if pack_result != OK:
		if bus:
			bus.log("ERROR: Failed to pack baseline household (error code %d)" % pack_result)
		return
	
	# Instantiate from PackedScene
	var new_household = packed.instantiate()
	if new_household == null:
		if bus:
			bus.log("ERROR: Failed to instantiate household from PackedScene")
		return
	
	new_household.name = "Household_%d" % (households.size() + 1)
	new_household.position = spawn_pos
	
	# Ensure processing is enabled
	new_household.set_process(true)
	new_household.set_physics_process(true)
	new_household.process_mode = Node.PROCESS_MODE_INHERIT
	
	# Add to scene (this triggers _ready() which initializes @onready vars)
	add_child(new_household)
	
	# Wait for next frame to ensure _ready() has completed
	await get_tree().process_frame
	
	# Verify household has required child nodes before proceeding
	if not new_household.has_node("Wallet") or not new_household.has_node("Inventory"):
		if bus:
			bus.log("ERROR: %s missing critical child nodes" % new_household.name)
		new_household.queue_free()
		return
	
	# Get component references using get_node_or_null for safety
	var new_inv = new_household.get_node_or_null("Inventory") as Inventory
	var new_wallet = new_household.get_node_or_null("Wallet") as Wallet
	var new_hunger = new_household.get_node_or_null("HungerNeed") as HungerNeed
	var new_cap = new_household.get_node_or_null("InventoryCapacity") as InventoryCapacity
	var new_food_stockpile = new_household.get_node_or_null("FoodStockpile") as FoodStockpile
	var new_food_reserve = new_household.get_node_or_null("FoodReserve") as FoodReserve
	var new_route = new_household.get_node_or_null("RouteRunner") as RouteRunner
	
	# Validate all critical components
	if not new_inv or not new_wallet or not new_hunger or not new_route:
		if bus:
			bus.log("ERROR: %s missing critical components after instantiation" % new_household.name)
			bus.log("  inv=%s, wallet=%s, hunger=%s, route=%s" % [
				"OK" if new_inv else "NULL",
				"OK" if new_wallet else "NULL", 
				"OK" if new_hunger else "NULL",
				"OK" if new_route else "NULL"
			])
		new_household.queue_free()
		return
	
	# Double-check wallet is valid Wallet instance before assigning
	if not is_instance_valid(new_wallet) or not new_wallet is Wallet:
		if bus:
			bus.log("ERROR: %s wallet component is invalid or wrong type" % new_household.name)
		new_household.queue_free()
		return
	
	# Set starting resources using inventory API (NOT direct dict assignment)
	new_wallet.money = 5000.0  # Match baseline starting money
	# Clear and set inventory properly
	new_inv.items.clear()
	new_inv.set_qty("bread", 20)  # Starting bread buffer
	
	# Re-bind components to ensure connections are fresh
	if new_cap:
		new_cap.bind(new_inv)
		new_inv.bind_capacity(new_cap)
	if new_food_stockpile:
		new_food_stockpile.bind(new_inv)
	
	# Wire event bus
	new_household.event_bus = bus
	if new_food_stockpile:
		new_food_stockpile.bind_shocks(shocks)
	
	# Wire HungerNeed with proper name
	if new_hunger:
		new_hunger.bind(new_household.name, new_inv, bus, calendar)
	
	# CRITICAL: Set market reference (required for buying)
	new_household.market = market
	
	# CRITICAL: Set locations (this starts the household's market/home loop)
	# This also binds FoodReserve internally and starts RouteRunner
	new_household.set_locations(new_home, market_node)
	
	# Track household in global array (used by tick updates)
	households.append(new_household)
	
	# Add to Godot groups if used (for group-based iteration)
	new_household.add_to_group("households")
	new_household.add_to_group("agents")
	
	# Log detailed spawn info for validation
	if bus:
		bus.log("Day %d: POP GROWTH - spawned %s at (%.0f, %.0f)" % [
			day, new_household.name, new_household.position.x, new_household.position.y
		])
		bus.log("  Initial: $%.2f, %d bread, target %d, reserve min %d" % [
			new_wallet.money, 
			new_inv.get_qty("bread"),
			new_food_stockpile.target_buffer if new_food_stockpile else 0,
			new_food_reserve.min_reserve_units if new_food_reserve else 0
		])
		bus.log("  Registered: households[%d], Market=%s, Home=%s, Processing=%s" % [
			households.size(),
			"SET" if new_household.market else "NULL",
			"SET" if new_household.home_location else "NULL",
			"ENABLED" if new_household.process_mode == Node.PROCESS_MODE_INHERIT else "DISABLED"
		])
		bus.log("  Route: target=%s, is_waiting=%s" % [
			new_route.target.name if new_route.target else "NULL",
			"YES" if new_route.is_waiting else "NO"
		])
		bus.log("  Phase: %s" % (
			"AT_MARKET" if new_household.phase == HouseholdAgent.Phase.AT_MARKET else "AT_HOME"
		))
	
	# Update prosperity tracking
	if prosperity:
		prosperity.record_spawn(day)
		if bus:
			bus.log("  Prosperity=%.2f (threshold=%.2f)" % [
				prosperity.prosperity_score, 
				prosperity.PROSPERITY_THRESHOLD_TO_GROW
			])



func _find_valid_spawn_position(market_pos: Vector2) -> Vector2:
	"""Find a spawn position that avoids overlapping with existing households."""
	var max_attempts = 20
	for attempt in range(max_attempts):
		# Random position within radius range
		var angle = randf() * TAU
		var radius = HOUSEHOLD_SPAWN_RADIUS_MIN + randf() * (HOUSEHOLD_SPAWN_RADIUS_MAX - HOUSEHOLD_SPAWN_RADIUS_MIN)
		var candidate_pos = market_pos + Vector2(cos(angle), sin(angle)) * radius
		
		# Check distance from all existing households
		var valid = true
		for household in households:
			if household != null:
				var dist = candidate_pos.distance_to(household.position)
				if dist < MIN_DISTANCE_FROM_HOUSEHOLDS:
					valid = false
					break
	...
	# Fallback: just use random position even if overlapping
	var angle = randf() * TAU
	var radius = (HOUSEHOLD_SPAWN_RADIUS_MIN + HOUSEHOLD_SPAWN_RADIUS_MAX) * 0.5
	return market_pos + Vector2(cos(angle), sin(angle)) * radius
